...
[Meshery](https://meshery.io/), the service mesh management plane was chosen as the tool to showcase the SMI specifications because:
...
### Learnings and Future Implementations

During the course of the program, I learned a lot more about service meshes, their architecture, the SMI spec's applications and how the APIs can be extended to fit any service mesh data plane and help test and analyze its abilities. The SMI specification is still relatively new to the service mesh landscape and needs more user tutorials and guides to familiarize both developers and users with its workings. I will continue to remain active in the [#smi](https://cloud-native.slack.com/?redir=%2Fmessages%2Fsmi) community after the conclusion of my GSoD project. The next steps for improvement to the SMI project should be:
